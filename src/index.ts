    ...
    switch (name) {
      ...
      case "voicememos_list": {
        ...
        try {
          ...
          if (files.length === 0) {
            return { content: [{ type: "text", text: "No voice memos found" }] };
          }
          ...
          return { content: [{ type: "text", text: `Voice Memos (${files.length}):\n\n${listing}` }] };
        } catch (error) {
          return { content: [{ type: "text", text: `Could not access Voice Memos folder. Error: ${error instanceof Error ? error.message : String(error)}\n\nExpected path: ${VOICE_MEMOS_PATH}` }] };
        }
      }
  ...
  tell process "Voice Memos"
    click button 1 of window 1
        ...
        try {
          ...
          return { content: [{ type: "text", text: "Started recording. Use voicememos_stop_recording to stop." }] };
        } catch {
          return { content: [{ type: "text", text: "Could not start recording. Please ensure Voice Memos is open and Accessibility permissions are granted." }] };
        }
      }
  ...
  tell process "Voice Memos"
    click button 1 of window 1
        ...
        try {
          ...
          return { content: [{ type: "text", text: "Could not stop recording. Please ensure Voice Memos is open and has an active recording." }] };
        }
      }
      ...
      case "voicememos_play": {
        ...
        try {
          ...
          return { content: [{ type: "text", text: `Could not play file: ${filename}. Make sure it exists in the Voice Memos folder.` }] };
        }
      }
